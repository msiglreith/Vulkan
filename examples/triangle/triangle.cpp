...
public:
	...
	void preparePipelines()
	{
		...
		shaderStages[0].module = loadSPIRVShader(getAssetPath() + "triangle.vert.spv");
#else
		...
		// Load binary SPIR-V shader
#if defined(VK_USE_PLATFORM_UWP_RKZ)
        shaderStages[1].module = loadSPIRVShader(getAssetPath() + "triangle.frag.spv");
#else
...
namespace Triangle {
    ...
    public:
        View(UINT_PTR sample) :
            example(reinterpret_cast<VulkanExample*>(sample))
        {}
        ...
        virtual void SetWindow(Windows::UI::Core::CoreWindow ^window) {
            example->window = reinterpret_cast<IUnknown *>(window);
        }
    ...
    public:
        ViewProvider(UINT_PTR example) :
            example(example)
        {}

        virtual IFrameworkView^ CreateView() {
            return ref new View(example);
        }
    private:
        UINT_PTR example;
    };
...
{
    auto example = new VulkanExample();
    auto triangle = ref new Triangle::ViewProvider(reinterpret_cast<UINT_PTR>(example));
    CoreApplication::Run(triangle);
    delete(example);
	return 0;
}
